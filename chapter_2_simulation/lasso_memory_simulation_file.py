...
def train(train_dataloader, lasso, optimizer, alpha = 0.3, criterion = nn.MSELoss()):
    alpha = 0.3
    lasso.train()
...
#         optimizer.zero_grad()
        ...
        loss.backward()
        optimizer.step()
    return lasso, optimizer


def evaluate(test_dataloader, lasso, alpha = 0.3, criterion = nn.MSELoss()):
    lasso.eval()
    losses = []
...
def pipeline(p, n):
    ...
    for epoch in range(num_epochs):
        # Forward pass
        lasso, optimizer = train(train_dataloader, lasso, optimizer)

        if epoch % 10 == 0:
            current_val_loss = evaluate(test_dataloader, lasso)
